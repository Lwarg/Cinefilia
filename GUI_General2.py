...
def register(user, DBpath):
    ...
    fileTestoLettura = open(''+ DBpath +'/users2.txt','r')
    fileTestoScrittura = open(''+ DBpath +'/users2.txt','a')
    numero_utenti = 0
...
def login(user, DBpath):
    ...
    # l'username viene appeso nel file users.txt che si trova nella cartella sincronizzata con google drive
    userEsistente = False
    utenti = open(''+ DBpath +'/users2.txt','r')
    for riga in utenti:
        if riga.strip().split(",")[1] == user:
            userEsistente = True
            utente = user
...
def cercaUtente(user):
    ...
    fileTesto = open(database+'/users2.txt','r')
    flag = 0
...
def visualizzaListaAmici():
    ...
    numeroAmici = 0
    fileUtenti = open(''+database+'/users2.txt','r')
